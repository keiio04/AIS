...
$db = get_db();
$stmt = $db->prepare("SELECT id, code, name, category FROM accounts");
$stmt->execute();
...
const lines = [
    { acc: accounts.find(a => a.name === 'Accounts Receivable'), dr: 10000, cr: 0 },
    { acc: accounts.find(a => a.name === 'Service Revenue'), dr: 0, cr: 10000 }
];
...
console.log("Result:", _detectGJMismatch(lines));
</script>
