...
$saveError = journal_vat_save($db, [
    ...
    'journal_id' => 'CRJ',
    'reference_prefix' => 'CRJ',
    'vat_mode' => 'output',
    'redirect' => 'cash_receipts_journal.php',
], $accountsList, $companyIsTaxRegistered);
if ($saveError !== null) {
    $error = $saveError;
}

journal_vat_delete($db, (int)$company_id, 'CRJ', 'cash_receipts_journal.php', 'Cash Receipts Journal');
...
function onAccountSearchInput(inputEl) {
    ...
    tr.querySelector('.account-id-input').value…424 tokens truncated…l.style.display = 'none'; }
}
...
function _detectMismatch(lines) {
    ...
    if (!hasCash) {
        ...
        return { journal: 'Cash Receipts Journal', url: null, reason: 'A Cash or Bank account is required for cash receipts.' };
    }
    return null;
}
