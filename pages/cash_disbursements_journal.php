...
function validateJournalLines($db, $company_id, $account_ids, $debits, $credits) {
    ...
    foreach ($validLines as $line) {
        ...
        if ($account['is_cash'] && $line['debit'] > 0) {
            $cashDebit = true;
        }
    }
    if (!$cashDebit) {
        return ['error' => 'Cash Receipts Journal requires a Cash or Bank account on the debit side.'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_entry' || $_POST['action'] === 'edit_entry') {
        ...
        if (isset($validation['error'])) {
            ...
            if ($isEdit) {
                $check = $db->prepare("SELECT id FROM journal_entries WHERE id = ? AND company_id = ? AND journal_id = 'CRJ' AND deleted_at IS NULL");
        ...
        if (!isset($error)) {
            ...
            try {
                if ($isEdit) {
                    $stmt = $db->prepare("UPDATE journal_entries SET reference_no = ?, date = ?, description = ?, is_taxable = ?, vendor_name = ? WHERE id = ? AND company_id = ? AND journal_id = 'CRJ'");
                ...
                $log_action = "$verb Cash Receipts Journal Entry | Ref: $ref_no | Amount: ₱" . number_format($validation['total_debit'], 2);
        ...
        $stmtDel = $db->prepare("UPDATE journal_entries SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND company_id = ? AND journal_id = 'CRJ'");
        $stmtDel->bind_param('ii', $delete_id, $company_id);
        $stmtDel->execute();

        $user_id = $_SESSION['user_id'];
        $log_action = "Moved Cash Receipts Journal Entry #$delete_id to Trash";
...
function _detectMismatch(lines) {
    ...
    const cashDebit = lines.some(l => _isCash(l.acc) && l.dr > 0);
