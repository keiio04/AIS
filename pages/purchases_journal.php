...
function validateJournalLines($db, $company_id, $account_ids, $debits, $credits) {
    ...
    if ($hasCash) {
        return ['error' => 'This entry contains a Cash/Bank account and belongs in the Cash Disbursements Journal, not the Purchases Journal.'];
    }
    if ($hasRevenue) {
        return ['error' => 'This entry contains a Revenue account and belongs in the Sales Journal, not the Purchases Journal.'];
    }
    if (!$hasPayable) {
        return ['error' => 'Purchases Journal requires at least one Payable or Liability account.'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_entry' || $_POST['action'] === 'edit_entry') {
        ...
        if (isset($validation['error'])) {
            ...
            if ($isEdit) {
                $check = $db->prepare("SELECT id FROM journal_entries WHERE id = ? AND company_id = ? AND journal_id = 'PJ' AND deleted_at IS NULL");
        ...
        if (!isset($error)) {
            ...
            try {
                if ($isEdit) {
                    $stmt = $db->prepare("UPDATE journal_entries SET reference_no = ?, date = ?, description = ?, is_taxable = ?, vendor_name = ? WHERE id = ? AND company_id = ? AND journal_id = 'PJ'");
                ...
                $log_action = "$verb Purchases Journal Entry | Ref: $ref_no | Amount: ₱" . number_format($validation['total_debit'], 2);
        ...
        $stmtDel = $db->prepare("UPDATE journal_entries SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND company_id = ? AND journal_id = 'PJ'");
...
$query = "
    ...
    WHERE e.company_id = ? AND e.deleted_at IS NULL AND e.journal_id = 'PJ'
...
function _detectMismatch(lines) {
    ...
    if (!hasPay)
        return { journal: 'Purchases Journal', url: null, reason: 'A Payable or Liability account is required for credit purchases.' };
    return null;
}
