...
$saveError = journal_vat_save($db, [
    ...
    'reference_prefix' => 'PJ',
    'vat_mode' => 'input',
    'redirect' => 'purchases_journal.php',
], $accountsList, $companyIsTaxRegistered);
if ($saveError !== null) {
    $error = $saveError;
}

journal_vat_delete($db, (int)$company_id, 'PJ', 'purchases_journal.php', 'Purchases Journal');
...
initJournalVat({ companyRegistered: companyIsTaxRegistered, rate: <?= json_encode(VAT_RATE) ?>, mode: 'input', accounts });
...
function onAccountSearchInput(inputEl) {
    ...
    tr.querySelector('.account-id-input').valu…561 tokens truncated…ector('.account-search-input');
