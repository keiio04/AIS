...
/* Google Sign-in Round Button & Divider */
.social-login-wrap {
  display: flex;
  justify-content: center;
  align-items: center;
...
.btn-google-circle:active {
  transform: translateY(0) scale(0.96);
}
.btn-google-circle svg {
  width: 22px;
  height: 22px;
  display: block;
}
...
<div class="page-wrap">
  ...
  <div class="auth-right">
    <div class="auth-form-wrap">
      ...
      <h2 id="form-title"><?= $mode === 'register' ? 'Create Account' : 'Sign In' ?></h2>
      ...
      <!-- ── SOCIAL LOGIN (GOOGLE CIRCLE BUTTON) ── -->
      <div class="auth-divider">
        <span id="auth-divider-text"><?= $mode === 'register' ? 'or sign up with' : 'or sign in with' ?></span>
      </div>
      <div class="social-login-wrap">
        <button type="button" class="btn-google-circle" id="btn-google-auth" onclick="handleGoogleSignIn()" title="Sign in with Google" aria-label="Sign in with Google">
          ...
          <span id="google-btn-text" style="display:none;"><?= $mode === 'register' ? 'Sign up with Google' : 'Sign in with Google' ?></span>
        </button>
      </div>
...
function switchTab(tab) {
  ...
  if (tab === 'login') {
    ...
    if (ft) ft.textContent = 'Sign In';
    if (gBtnText) gBtnText.textContent = 'Sign in with Google';
    if (authDividerText) authDividerText.textContent = 'or sign in with';
  } else {
    ...
    if (authDividerText) authDividerText.textContent = 'or sign up with';
  }
}
